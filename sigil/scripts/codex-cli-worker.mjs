...
process.stdin.on('end', () => {
  ...
  const taskId = task.task_id ?? task.body?.task_id;
  if (!taskId) { process.stderr.write('Task JSON is missing required task_id\n'); process.exit(1); }
  ...
  child.on('close', (code) => {
    ...
    process.stdout.write(JSON.stringify({ task_id: taskId, status: 'completed', processing: 'codex_cli_subscription_or_login', summary: stdout.trim() }));
  });
});
