...
test('synthetic triage ingress produces one signed envelope and receipt', async () => {
  ...
  const input = {
    ...
    registry: { inboxMappings: [
      ...
      { providerInboxId: 'inbox_c', endpointId: 'ep_iron', webhookSecretId: 'wh_iron', workflowPolicy: ['test'] },
    ] },
    secretStore: { current: () => ({ withWebhookSecret: (_id, callback) => callback({ withValue: (fn) => fn('synthetic-webhook') }), withForwardingToken: (_alias, callback) => callback({ withValue: (fn) => fn('I'.repeat(22)) }) }) },
