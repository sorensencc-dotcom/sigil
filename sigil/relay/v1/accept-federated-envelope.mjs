...
export async function acceptFederatedEnvelope(body, headers, options) {
  ...
  return repository.withTransaction(async (client) => {
    ...
    if (repository.recordAuditEvent) {
      await repository.recordAuditEvent({ eventType: 'federation.inbound_accepted', subjectId: persisted?.message_id ?? result.message_id, endpointId: recipientId, outcome: 'accepted', reason: null, payload: { origin_domain: originDomain, recipient_domain: relayDomain }, now, client });
    }
