...
- `sigil inbox --identity <path> --relay-url <url> [--watch]` — polls the relay's inbox, prints messages, acknowledges delivery.
...
- **Not hosted.** `sigil relay up` is a foreground process on localhost. Nothing survives a restart (in-memory store). No one outside your machine can reach it without you exposing the port yourself, which nothing here helps you do safely.
- **Not integrated into any chat UI.** Sending a message does not make it appear inside an actual Claude or Codex conversation. `sigil inbox` is a terminal poll loop you run yourself. There is no push notification, no webhook into either product's UI, no "new message" indicator anywhere but this terminal.
...
4. **Push, not poll** — `relay/v1/stream-server.mjs` already exists (WebSocket notify-on-delivery) but the CLI doesn't use it; `sigil inbox --watch` is a dumb poll loop. A real product wants push delivery into whatever surfaces the message.
...
- Wire `sigil inbox --watch` to the existing WebSocket stream server instead of polling.
